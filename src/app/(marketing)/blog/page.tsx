...
export default function Page(props) {
  const posts = allPosts
    .filter((post) => (typeof props.searchParams?.draft === 'string' && !isProduction) || post.published)
    .sort((a, b) => compareDesc(new Date(a.date), new Date(b.date)))
  return (
    <div className="container mx-auto max-w-4xl px-4 py-6 lg:px-0 lg:py-10">
      <div className="flex flex-col items-start gap-4 md:flex-row md:justify-between md:gap-8">
        <div className="flex-1 space-y-4">
          <h1 className="inline-block text-4xl font-extrabold tracking-tight text-slate-900 lg:text-5xl">Blog</h1>
          <p className="text-xl text-slate-600">We write about things.</p>
        </div>
      </div>
